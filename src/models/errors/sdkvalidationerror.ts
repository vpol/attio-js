...
export function formatZodError(err: z.ZodError, level = 0): string {
  ...
  for (const issue of err.issues) {
    ...
    switch (issue.code) {
      ...
      case "invalid_value": {
        append(`│     Allowed: ${issue.values.join(", ")}`);
        break;
      }
      ...
      case "invalid_union": {
        const len = issue.errors.length;
        append(
          `│   ✖︎ Attemped to deserialize into one of ${len} union members:`,
        );
        issue.errors.forEach((issues, i) => {
          append(`│   ✖︎ Member ${i + 1} of ${len}`);
          const memberErr = new z.ZodError(issues);
          append(`${formatZodError(memberErr, level + 1)}`);
        });
      }
    }
  }
